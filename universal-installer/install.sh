...
install_base() {
  ...
  if ! id "$HOMEOS_ADMIN_USER" &>/dev/null; then
    ...
    passwd -e "$HOMEOS_ADMIN_USER"  # Force password change on first login
  fi
...
install_node() {
  ...
  if [[ ! -x "${HOMEOS_ADMIN_HOME}/.bun/bin/bun" ]]; then
    log "Installing Bun..."
    su - "$HOMEOS_ADMIN_USER" -c "curl -fsSL https://bun.sh/install | bash"
  fi
...
services:
  homeassistant:
    ...
    network_mode: host
    environment:
      - TZ=${TIMEZONE:-$(timedatectl show --property=Timezone --value)}
    volumes:
      - ha-config:/config
